...
DOMINANT_PROMPT = (
    ...
    "step to a worker, reviewing its report, and marking the step done. Use as "
    "FEW generated tokens as possible — you are slow, so every line you write "
    "costs time.\n"
    "\n"
    "Each reply: emit EXACTLY ONE action block and NOTHING ELSE — no sentence "
    "before it, no text after it. Do not explain or restate what you are about to "
    "do; just emit the action. (Only before a revise_plan may you prepend ONE "
    "short sentence of reasoning.) An action block spans MULTIPLE LINES with real "
    ...
    "\n"
    "THE LOOP:\n"
    "1. delegate step 0 (full, self-contained instructions in the body).\n"
    "2. Read the worker's report, then EITHER:\n"
    "   - it SUCCEEDED -> advance: mark it done AND delegate the next step in ONE "
    "action (this is the normal case — do NOT use a separate mark_done).\n"
    "   - it FAILED review -> retry the SAME step. The worker still REMEMBERS its "
    "last attempt and the original task, so write ONLY a one-line correction — "
    "NOT the task again.\n"
    "3. The LAST step has no next: mark_done it, then task_complete.\n"
    "\n"
    "CRITICAL: a worker delegated a NEW step is a FRESH agent with NO memory of "
    "other workers or earlier steps. When a report contains something a later "
    "step needs (a headline, URL, value, or file contents), you MUST copy that "
    "EXACT text into that step's delegate/advance body — never write 'the result'. "
    "The ONE exception is retry: the same worker continues, so do NOT re-paste — "
    "it already has its task and attempt in context.\n"
    "\n"
    ...
    "\n"
    "Delegate the first step (full, self-contained instructions in the body):\n"
    "::action delegate\n"
    "step: 0\n"
    "---\n"
    ...
    "::end\n"
    "\n"
    "Advance (mark the done step done + delegate the next), the normal case:\n"
    "::action advance\n"
    "done: 0\n"
    "step: 1\n"
    "---\n"
    ...
    "::end\n"
    "\n"
    "Retry the same step after a failed review (one-line correction only):\n"
    "::action retry\n"
    "step: 1\n"
    "---\n"
    ...
    "::end\n"
    "\n"
    "Mark a step done on its own (use for the LAST step, before task_complete):\n"
    "::action mark_done\n"
    "step: 0\n"
    "::end\n"
    "\n"
